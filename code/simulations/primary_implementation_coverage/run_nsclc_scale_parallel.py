"""Run the eight NSCLC-scale coverage shards, then aggregate."""
...
RUNNER=HERE/"run_nsclc_scale.py"
LOG=HERE/"nsclc_scale_logs"; STATUS=HERE/"nsclc_scale_status.json"
...
def main():
    ...
    while pending or running:
        while pending and len(running)<8:
            nt,seed=pending.pop(0);tag=f"nt{nt}_seed{seed}";env=os.environ.copy();env.update({"COVERAGE_TARGET_SIZE":str(nt),"COVERAGE_SEED":str(seed)})
    ...
    env=os.environ.copy();env["COVERAGE_AGGREGATE_ONLY"]="1"
