"""Censoring-floor and nuisance-fit diagnostics.
...
NSCLC = Path(os.environ.get("DIAGNOSTIC_NSCLC_INPUT_ROOT", HERE / "restricted_inputs" / "nsclc"))
ROT = Path(os.environ.get("DIAGNOSTIC_ROTTERDAM_GBSG_INPUT_ROOT", HERE / "restricted_inputs" / "rotterdam_gbsg"))
OUT = Path(os.environ.get("DIAGNOSTIC_OUTPUT_ROOT", HERE / "diagnostic_results"))
MODELS = ("PET_radiomic_candidate", "PET_radiomic_candidate_SRDO")
SITES = ("Stanford", "VA")
SEEDS = (42, 52, 62, 72)
G_MIN = 0.02
BOOT = int(os.environ.get("DIAGNOSTIC_BOOTSTRAP", "999"))
WORKERS = int(os.environ.get("DIAGNOSTIC_WORKERS", "8"))
...
def one_job(job):
    ...
    rng = np.random.default_rng(stable_seed("direct-versus-separate", dataset, contrast, seed, BOOT))
...
def jobs():
    ...
    for contrast, base in (
        ("ER-only", ROT),
        ("supporting_clinical_candidate", ROT),
    ):
        for seed in SEEDS:
...
def main():
    ...
    details = OUT / "configuration_diagnostics.csv"; frame.to_csv(details, index=False)
    ...
    summary_path = OUT / "diagnostic_summary.csv"; summary.to_csv(summary_path, index=False)
    manifest = {
        "status": "PASS",
