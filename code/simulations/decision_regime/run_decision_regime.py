"""Decision-regime study for the direct Brier-risk contrast.

Evaluates five prespecified robustness-value regimes:
...
Configuration hash and effective smoke/shard settings are recorded separately.
"""
...
CFG=json.load(open(os.path.join(HERE,"configuration.json")))
SEEDS=CFG["seeds"]; REPS=int(os.environ.get("REGIME_REPLICATES",CFG["reps_total"])); BOOT=int(os.environ.get("REGIME_BOOTSTRAP",CFG["boot"]))
...
ACT_SEEDS=[int(os.environ["REGIME_ONLY_SEED"])] if "REGIME_ONLY_SEED" in os.environ else SEEDS
ACT_CENS=[os.environ["REGIME_ONLY_CENSORING"]] if "REGIME_ONLY_CENSORING" in os.environ else CENS
ACT_REG=[os.environ["REGIME_ONLY_SETTING"]] if "REGIME_ONLY_SETTING" in os.environ else list(REGIMES)
BENCH=os.environ.get("REGIME_BENCHMARK")=="1"
IS_SHARD=any(e in os.environ for e in ["REGIME_ONLY_SEED","REGIME_ONLY_CENSORING","REGIME_ONLY_SETTING"])
AGG_ONLY=os.environ.get("REGIME_AGGREGATE_ONLY")=="1"
OUT=os.path.join(HERE,"decision_regime_results",f"run_{CFG_HASH}"); os.makedirs(OUT,exist_ok=True)
_manifest={"requested_reps":CFG["reps_total"],"effective_reps":REPS,"requested_boot":CFG["boot"],"effective_boot":BOOT,
           "environment_overrides":{e:os.environ.get(e) for e in["REGIME_REPLICATES","REGIME_BOOTSTRAP","REGIME_ONLY_SEED","REGIME_ONLY_CENSORING","REGIME_ONLY_SETTING","REGIME_BENCHMARK","REGIME_AGGREGATE_ONLY"] if e in os.environ},
...
if IS_SHARD and not AGG_ONLY:
    _tag="_".join(filter(None,[os.environ.get("REGIME_ONLY_SETTING"),os.environ.get("REGIME_ONLY_CENSORING"),os.environ.get("REGIME_ONLY_SEED")]))
    json.dump(_manifest,open(os.path.join(OUT,f"execution_{_tag}.json"),"w"),indent=2)
else:
    json.dump(_manifest,open(os.path.join(OUT,"execution.json"),"w"),indent=2)
...
def main():
    ...
    for reg in ACT_REG:
        ...
        for cens in ACT_CENS:
            ...
            json.dump(agg,open(os.path.join(OUT,f"{reg}_{cens}.json"),"w"),indent=2);print(json.dumps(agg,indent=2))
