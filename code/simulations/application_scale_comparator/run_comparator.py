"""Application-scale comparator and mirror simulation engine.

The implementation uses deterministic seeds, distinguishes the
data-generating and analysis sensitivity classes, and implements the
prespecified source-outcome-regression misspecification condition.
...
gamma-indexed identified sets, and (when requested) obtains method-specific
...
REPS = int(os.environ.get("SIMULATION_REPLICATES", "1000"))
WITH_CI = os.environ.get("SIMULATION_WITH_CI", "1") == "1"
BOOT = int(os.environ.get("SIMULATION_BOOTSTRAP", "199" if WITH_CI else "0"))
ONLY_SCENARIO = os.environ.get("SIMULATION_SCENARIO", "").strip()
BASE_SEED = int(os.environ.get("SIMULATION_SEED", "42"))
DEFER_COST = float(os.environ.get("SIMULATION_DEFER_COST", "0.003"))

N_SOURCE = int(os.environ.get("SIMULATION_SOURCE_SIZE", "114"))
N_TARGET = int(os.environ.get("SIMULATION_TARGET_SIZE", "30"))
TAU = 1.0
CENSOR_SOURCE_DESIGN = float(os.environ.get("SIMULATION_SOURCE_CENSORING", "0.69"))
CENSOR_SCALE = float(os.environ.get("SIMULATION_CENSORING_SCALE", "0.70"))
...
def main():
    ...
    # Optional output root makes clean-checkout smoke/full regeneration possible
    # without changing the locked default used for the manuscript run.
    output_root = Path(os.environ.get("SIMULATION_OUTPUT_ROOT", str(HERE / "comparator")))
    out = output_root / f"run_{config_hash}"
