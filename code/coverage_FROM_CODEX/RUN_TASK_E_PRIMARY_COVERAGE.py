"""Task E: coverage of the exact conditional Cox--Breslow primary pipeline.

Shard with TASK_E_NT and TASK_E_SEED.  Aggregate with TASK_E_AGG_ONLY=1.
...
import RUN_TASK_F_DIAGNOSTICS as diag


HERE = Path(__file__).resolve().parent
OUT = HERE / "task_e_primary_coverage"
SEEDS = (42, 52, 62, 72)
N_TARGETS = (60, 80)
N_SOURCE = 114
REPS_TOTAL = int(os.environ.get("TASK_E_REPS", "1000"))
BOOT = int(os.environ.get("TASK_E_BOOT", "199"))
ONLY_SEED = int(os.environ["TASK_E_SEED"]) if "TASK_E_SEED" in os.environ else None
ONLY_NT = int(os.environ["TASK_E_NT"]) if "TASK_E_NT" in os.environ else None
AGG_ONLY = os.environ.get("TASK_E_AGG_ONLY") == "1"
...
def aggregate():
    ...
    frame=pd.DataFrame(rows); result=OUT/"task_e_coverage_summary.csv"; frame.to_csv(result,index=False)
    manifest={"status":"PASS_TASK_E","created_utc":datetime.now(timezone.utc).isoformat(),
